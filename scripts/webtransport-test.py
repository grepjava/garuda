...
"""WebTransport checks against an independent implementation.
    ...
    python3 scripts/webtransport-test.py [path-to-garuda-conformance]
...
# The routes under test are the conformance app's (Sources/GarudaConformance).
...
class Server:
    def __init__(self, *args):
        ...
        cmd = [BIN, "--port", str(self.port), "--log-level", "error",
               "--http3", "--tls-cert", cert, "--tls-key", key] + list(args)
...
class Client(QuicConnectionProtocol):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.body_ended = set()
        self.datagrams = []
        self.datagram_sessions = []      # (session id, data) as each arrived
        self.resets = {}                 # stream id -> error code
        self.settings = None
        # Every byte each stream carried, before HTTP/3 reads it: a session's
        # CONNECT stream carries capsules, which aioquic does not report.
        self.raw = {}
    ...
    def open_raw_stream(self, stream_id, session_id, data=b"", end=True):
        """A bidirectional session stream on an ID of our choosing, which
        aioquic's own call does not allow: it names a session the server may
        not have yet."""
        with self._http._get_or_create_stream(stream_id) as state:
            state.frame_type = WEBTRANSPORT_STREAM
            state.session_id = session_id
        self._quic.send_stream_data(
            stream_id, encode_uint_var(WEBTRANSPORT_STREAM) + encode_uint_var(session_id) + data,
            end_stream=end)
        self.transmit()
        return stream_id
    ...
    def quic_event_received(self, event):
        ...
        for http_event in self._http.handle_event(event):
            if isinstance(http_event, HeadersReceived):
                ...
                self.datagram_sessions.append((http_event.stream_id, http_event.data))
        self._events.set()
...
async def aborted_streams():
    ...
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
            ...
            is_("and what arrived before the reset is kept, marked as aborted",
                body, b"aborted:half")


async def early_streams():
    """Streams that name a session before its CONNECT has been answered.

    QUIC delivers each stream on its own, so a client's first stream can reach
    the server ahead of the CONNECT it belongs to. Those are held and handed
    over when the session is accepted -- but only so many, or a peer could
    park streams against sessions it never opens.
    """
    print("\nStreams ahead of their session")
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
                           create_protocol=Client) as client:
            session = client._quic.get_next_available_stream_id()
            early = client.open_raw_stream(session + 4, session, data=b"first")
            # Long enough that the server has read the stream, found no session
            # and held it, before the CONNECT goes out.
            await asyncio.sleep(0.3)
            client.connect_session("/wt", stream_id=session)
            headers = await client.await_session(session)
            is_("a session whose stream came first is accepted", headers.get(b":status"), b"200")
            is_("and the stream that came first is handed to it",
                await client.wait_stream(early), b"echo:first")

        async with connect("127.0.0.1", server.port, configuration=configuration(),
                           create_protocol=Client) as client:
            # A session that is never opened, and more streams for it than the
            # server holds.
            # As many streams as the connection allows at once (128), so the
            # rejected ones have to be given back before anything else opens.
            first = client._quic.get_next_available_stream_id()
            nowhere = first + 4000
            ids = [client.open_raw_stream(first + 4 * i, nowhere, data=b"x") for i in range(128)]
            await client.wait_for(lambda: len(client.resets) >= 96, 10)
            rejected = [s for s in ids
                        if client.resets.get(s) == WEBTRANSPORT_BUFFERED_STREAM_REJECTED]
            is_("streams past the limit are reset as rejected", len(rejected), 96)
            is_("and they are the ones past it", rejected, ids[32:])
            await asyncio.sleep(0.3)
            try:
                status, _ = await client.get("/")
            except Exception as error:
                status = error
            is_("and a rejected stream's place can be taken by another", status, b"200")
...
async def datagrams():
    ...
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
            ...
            # A second session, whose ID is not 0, so that a quarter stream ID
            # and a stream ID differ.
            other = client.connect_session("/wt")
            await client.await_session(other)
            client.send_datagram(other, b"second")
            await client.wait_for(lambda: b"echo:second" in client.datagrams, 10)
            is_("a datagram names its session by the quarter stream ID",
                [s for s, d in client.datagram_sessions if d == b"echo:second"], [other])
...
async def closing():
    ...
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
            ...
            # The peer's close capsule ends the session, the handler waiting on
            # it wakes and returns, and it can see the code and the reason.
            report = client.connect_session("/wt-report")
            await client.await_session(report)
            client.close_session(report, code=9, reason=b"bye now")
            seen = b""
            for _ in range(50):
                _, seen = await client.get("/wt-last-close")
                if seen:
                    break
                await asyncio.sleep(0.1)
            is_("a handler waiting on a session the peer closed returns, with its code and reason",
                seen, b"9 bye now")
...
async def routes():
    ...
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
                           create_protocol=Client) as client:
            known = set(client.stream_ended)
            room = client.connect_session("/wt-room/lobby")
            headers = await client.await_session(room)
            is_("a typed session route accepts", headers.get(b":status"), b"200")
            is_("with the headers its middleware added", headers.get(b"x-room"), b"yes")
            _, body = await client.wait_new_stream(known)
            is_("a path parameter reaches the handler", body, b"welcome to lobby")

            missing = client.connect_session("/wt-nowhere")
            headers = await client.await_session(missing)
            is_("an unrouted path is 404", headers.get(b":status"), b"404")

            bad_param = client.connect_session("/wt-count/seven")
            headers = await client.await_session(bad_param)
            is_("an extractor that refuses refuses the session",
                headers.get(b":status"), b"400")

            status, _ = await client.get("/wt")
            is_("a GET to a session path is 405", status, b"405")
            request = [s for s in client.headers if client.headers[s].get(b":status") == b"405"]
            is_("and Allow names CONNECT",
                client.headers[request[-1]].get(b"allow") if request else None, b"CONNECT")


async def close_capsule():
    print("\nThe close capsule")
    with Server() as server:
        async with connect("127.0.0.1", server.port, configuration=configuration(),
                           create_protocol=Client) as client:
            session = client.connect_session("/wt-close")
            await client.await_session(session)
            payload = (7).to_bytes(4, "big") + b"asked to"
            expected = encode_uint_var(CLOSE_WEBTRANSPORT_SESSION) + encode_uint_var(len(payload)) + payload
            await client.wait_for(lambda: expected in client.raw.get(session, b""), 10)
            data = client.raw.get(session, b"")
            check("the application's code and reason reach the peer",
                  data.endswith(expected), data)

            # A handler that returns without closing has its session closed
            # for it, with code 0.
            returned = client.connect_session("/wt-return")
            await client.await_session(returned)
            closed = encode_uint_var(CLOSE_WEBTRANSPORT_SESSION) + encode_uint_var(4) + (0).to_bytes(4, "big")
            await client.wait_for(lambda: closed in client.raw.get(returned, b""), 10)
            check("a handler that returns closes its session",
                  client.raw.get(returned, b"").endswith(closed), client.raw.get(returned, b""))


def main():
    if not os.path.exists(BIN):
        sys.stderr.write("no garuda-conformance binary at %s\n" % BIN)
        return 2
