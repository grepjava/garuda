#!/usr/bin/env bash
# The shared response cache's own tests, compiled against garuda_cache.c:
...
${CC:-cc} -std=c11 -O1 -g -Wall -Wextra -Werror -DPG_CACHE_TESTING \
    -I "$ROOT/Sources/CGaruda/include" \
    "$ROOT/Sources/CGaruda/garuda_cache.c" "$ROOT/scripts/cache_unit.c" \
    -o "$OUT/cache-unit"
"$OUT/cache-unit"
