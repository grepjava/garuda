/* UDP for QUIC. See garuda_udp.h for why this is not just read/write.
 *
...
/* pg_set_nonblock / pg_set_cloexec live in garuda_sys.c. */
...
int pg_udp_gso_supported(int fd) {
    ...
    /* GARUDA_UDP_GSO=0 turns it off, so the one-datagram-per-send path the
     * tests would otherwise never reach on Linux can still be exercised. */
    const char *env = getenv("GARUDA_UDP_GSO");
