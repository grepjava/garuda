...
public struct Worker: ~Copyable {
    ...
    public init(config: ServerConfig, listenFD: Int32, poller: Poller) {
        ...
        let n = max(config.maxConnections, 1)
    ...
    public mutating func destroy() {
        ...
        outbound?.destroy()
        outbound = nil
        for (_, clientTLS) in outboundTLS { av_tls_ctx_free(clientTLS) }
    ...
    mutating func handleReadable(_ slot: Int) {
        ...
        case .http2:
            ...
            return
        case .dispatching:
            // The body is complete; extra bytes are a pipelined request and
            // wait in the socket until this response finishes.
    ...
    mutating func failRequest(_ slot: Int, status: Int) {
        ...
        HTTPResponseWriter.writeError(&c.pointee.write, status: status,
                                      closeConnection: true, dateCache: dates, serverName: "garuda",
