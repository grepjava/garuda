...
extension Worker {
    ...
    mutating func beginHTTP3(_ connection: QUICConnection) {
        ...
        host.withUnsafeBufferPointer { buffer in
            ...
            base.withMemoryRebound(to: UInt8.self, capacity: n) { bytes in
                ...
                c.pointee.remoteAddr.write(UnsafePointer(bytes), n)
            }
        }
    ...
    mutating func openH3Stream(parent: Int, _ h3: H3Connection, streamID: UInt64) -> Int {
        ...
        if p.pointee.remoteAddr.readableBytes > 0 {
            s.pointee.remoteAddr.write(
                UnsafePointer(p.pointee.remoteAddr.readPointer),
                p.pointee.remoteAddr.readableBytes)
        }
        s.pointee.remotePort = p.pointee.remotePort
