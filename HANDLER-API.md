...
- **Streaming request bodies and responses**, with backpressure reaching the producer: `write` reports a full connection above `--write-high-water`, and a reader waits for more body. Responses landed 2026-09-17. `response.stream()` sends the head through the ordinary sink, with a `streaming` flag that leaves the response open, and returns a `ResponseBodyWriter`. A typed handler returns `StreamingBody`, whose producer the task runs once the handler returns. The body is framed as it is written: chunked, close-delimited on HTTP/1.0, or DATA frames. A write waits above the high-water mark on a continuation in the slot, and `resumeWriterIfDrained` wakes it from every flush and QUIC ACK. The wake goes onto the executor and never runs inside the flush. Cancellation reaches the writer through `cancelOps`, and a stalled reader is swept after `--request-timeout`. Returning finishes the body; a throw resets the stream or closes the connection. `EventStream` builds on it. Request bodies are still read whole.
...
- **Server-sent events.** Landed 2026-09-17 with streamed responses, below.
