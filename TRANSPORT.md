...
### Kernel TLS: not available

The TLS record layer is a vendored BoringSSL, which has no kernel TLS, so
every build encrypts in the process. `--ktls` and `--no-ktls` are accepted and
ignored so that an existing command line keeps working, and the server says at
start-up that it is encrypting in the process.

What it did while OpenSSL held the record layer: a `--static-dir` file went
out over HTTPS/1.1 with `SSL_sendfile` rather than being read and encrypted,
and an idle HTTPS connection could be handed to another worker, because with
...
never moves, on any build: HTTP/2 (its HPACK tables and streams belong to the
worker), WebSocket, a streamed response or body, and HTTP/3.
